...
- 日期：2026-07-22
- 状态：G1 运行时视觉评审已完成主体项；三流派 5 秒区分与生产许可/工时签字待完成
...
| --- | ---: | --- | --- | --- |
| 非 Token 随从 | 64 | 配置、名称、文案、数值和语义 `artId`；不熄炉王已映射工程样板 Sprite | 63 张无专属 Sprite；当前样板生产许可未确认；`audioId` 全空 | 12 张专属样板插画，其余使用种族回退图 |
...
| 位图/Sprite | 14 | 工程样板 | 2 张插画、8 张立牌部件（含普通银灰/金色双框）、旧框 2 张与旅团绘本普通/金色卡框 2 张；生产许可待确认 |
...
| --- | ---: | --- | --- | --- | --- | --- | --- |
| M-F01 | 1 | 铸魂盾侍 / `forge_soul_shield_squire` | 1/3；2/6 | 嘲讽、开场护盾、金色左侧授盾 | `card_minion_forge_soul_shield_squire.png` | G1 工程样板已接入 Catalog、CardView 与 `PF_BattleStandee`；生产许可和最终视觉评审待确认 | P0 |
...
| M-F04 | 5 | 不熄炉王 / `undying_furnace_king` | 6/8；12/16 | 嘲讽、开场护盾、连续护盾转移、2/4 次上限 | `card_minion_undying_furnace_king.png` | v0.4 工程样板已在 `PF_BattleStandee` 完成 Unity 运行验证；最终评审与生产许可待确认 | P1 |
...
| D-03 | `PresentationSpriteCatalog` | 样板接入 | 已完成两张样板插画、旅团绘本普通/金色卡框和八张立牌部件映射；普通立牌使用独立银灰框、金色沿用暖金框；重复/全量缺失校验待补 | P0 |
...
| D-07 | 资产来源台账 | v0.1 已建立 | 持续登记作者/工具/许可/版本/哈希/评审；未清许可资产仅限工程样板 | P0 |
...
| `card_minion_undying_furnace_king` | 随从插画 | 用户提供 / Codex 导出 | ChatGPT（后端具体模型版本不可获得）、Pillow | `ui-concepts/ChatGPT Image 2026年7月22日 16_23_30.png` | 生产许可待项目方确认；仅限内部工程样板 | 2026-07-22 | 1024×1024 缩放导出、Artwork Mask 裁切验证 | v01 | G1 待评审 | 不标 Runtime Ready，不用于最终发布 |
| `card_frame_normal` | 公共普通框 | 用户提供 / Codex 修图 | ChatGPT（后端具体模型版本不可获得）、`extract_card_frame_alpha.py` | `ui-concepts/phase-9b/card-frames/shared-card-frame-normal-alpha-master-v0.1.png` | 生产许可待项目方确认；仅限内部工程样板 | 2026-07-22 | 棋盘格分离、真实 Alpha、边缘去白 | v01 | G1 待评审 | 与运行时导出哈希一致 |
| `card_frame_golden` | 公共金色框 | 用户提供参考 / Codex 修图 | ChatGPT（后端具体模型版本不可获得）、`create_golden_card_frame.py` | `ui-concepts/phase-9b/card-frames/shared-card-frame-golden-alpha-master-v0.1.png` | 生产许可待项目方确认；仅限内部工程样板 | 2026-07-22 | 古金材质迁移、裂纹/高光、复制普通框 Alpha | v01 | G1 待评审 | 静态框完成；Unity 流光材质未制作 |
| `card_frame_storybook_normal_v2` | 旅团绘本普通框 | Codex 生成/接入 | 内置 GPT Image 工作流、色键抠图工具 | `ui-concepts/phase-9b/card-frames/shared-card-frame-storybook-normal-chroma-v0.2.png` | 生产许可待项目方确认；仅限内部工程样板 | 2026-07-23 | 以旧框为几何目标、v0.3 Style Tile 为风格参考；洋红色键转真实 Alpha | v02 | G1 运行时评审通过 | 已替代旧黑银框用于 `PF_Card`，许可确认前不标 Runtime Ready |
| `card_frame_storybook_golden_v2` | 旅团绘本金色框 | Codex 生成/接入 | 内置 GPT Image 工作流、色键抠图工具 | `ui-concepts/phase-9b/card-frames/shared-card-frame-storybook-golden-chroma-v0.2.png` | 生产许可待项目方确认；仅限内部工程样板 | 2026-07-23 | 从普通框受控派生，保留纸面与靛蓝缝线，仅增强局部金箔 | v02 | G1 运行时评审通过 | 与普通框共享几何；无单卡分支 |
