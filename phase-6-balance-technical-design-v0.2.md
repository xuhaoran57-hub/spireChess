...
版本：0.2 设计稿
状态：最新六套完整阵容已按“五回合成长、其中三回合实际触发亡语永久收益”校准到 fixture v0.3；`R8-best-lineups-deathrattle3-dotnet` 完整 S0 已通过，待处理 B03-N、B01-H/B04-H 与 B02-H/B06 并进行 S2 人工复核
Unity：2022.3.62f3c1
内容基线：`content-release 5.1.0`，64 个非 Token 随从、3 个 Token、16 张法术
自动化基线：上次 Unity EditMode 111 / 111、PlayMode 14 / 14；本轮无 Unity 环境，.NET 编译零警告，`R8-best-lineups-deathrattle3-dotnet` 8,400 场 S0 首跑与同种子复跑通过
...
| `B06_REFRESH` | 最终商店成功主动刷新 4 次 | 最终商店成功主动刷新 5 次 | `N`：槽 0 获得下一战护盾；`H`：槽 0、1 获得下一战护盾 |
...
- `B03_SUMMON` 的繁茂是构筑级玩家状态，当前 provisional 快照固定为 N 4 层、H 8 层；这只是开战层数而不是上限，战中仍可继续增长；
