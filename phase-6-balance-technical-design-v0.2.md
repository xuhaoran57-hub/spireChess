...
版本：0.2 设计稿
状态：方案已定稿，待补齐统计实现并执行 R0 基线
Unity：2022.3.62f3c1
内容基线：`content-release 5.1.0`，64 个非 Token 随从、3 个 Token、16 张法术
自动化基线：EditMode 93 / 93、PlayMode 14 / 14
...
| `B03_SUMMON` | 0 | `fox_den_matriarch` | 狐群巢母 | +4/+5 | 8/10 | 16/20 |
| `B03_SUMMON` | 1 | `hundred_song_herd` | 百鸣兽群 | +3/+4 | 7/8 | 14/16 |
| `B03_SUMMON` | 2 | `ten_thousand_hoof_surge` | 万蹄奔潮 | +5/+4 | 12/10 | 24/20 |
...
| `B05_SPELL` | 最终商店使用 2 张法术 | 最终商店使用 4 张法术 | `N`：槽 0 获得下一战护盾，槽 1 获得临时 +2/+4；`H`：槽 0 获得下一战护盾，槽 1、2 各获得临时 +2/+4 |
| `B06_REFRESH` | 最终商店成功主动刷新 4 次 | 最终商店成功主动刷新 6 次 | `N`：槽 0、4 获得下一战护盾；`H`：五个槽位均获得下一战护盾 |
...
- `B06_REFRESH_N` 将星环司库的随机护盾固定落在槽 4，槽 0 护盾来自天穹契约者；这是夹具输入的一部分，真实单局仍保留随机目标；
...
- 对 `N` 快照使用 `initialAttack = 12`、`initialHealth = 500`；
- 对 `H` 快照使用 `initialAttack = 24`、`initialHealth = 1000`，使压力随面板等比例放大；
- 不带额外关键词，按正常攻击顺序施压；
...
| 护盾 | 普通/金色不熄炉王转移次数超过 3/6，或次数跨战斗继承 |
...
- `B01_SHIELD`：普通/金色炉王单场转移不超过 3/6；不存在无限护盾循环。
...
- [ ] 将六套 `N/H` 构筑和木桩定义为版本化夹具；
- [ ] 同时定义纯输出木桩和 `N/H` 机制压力板；
- [ ] 增加夹具 ID、卡牌 ID、最终身材、金色状态和 `SourceInstanceId` 校验；
- [ ] 将第 3.6 节核心映射实现为版本化离线分类器，并覆盖 `Mixed` / `Unclassified`；
- [ ] 扩展 `BattleSimulationResult` 或独立诊断收集器，提供第 5.3 节计数；
- [ ] 扩展 `BattleBatchRunner`，输出逐场样本和镜像聚合；
- [ ] 增加确定性哈希与同种子复跑测试；
- [ ] 建立 CSV/NDJSON 聚合器并校验 schema 版本。

### P0：完整单局遥测

- [ ] 新增 `RunEnded`、`ShopSnapshot`、`Turn10Snapshot`、`EventChoiceResolved`、`RewardChoiceResolved` 和扩展 `BattleCompleted`；
- [ ] 区分付费/免费刷新，记录购买、出售、施法、升级和金币浪费；
- [ ] 记录核心首次获得、第二核心形成、三连和定向发现；
- [ ] 提供人工决策摘要模板，不实现完整命令重放；
- [ ] 从 20 份 NDJSON 自动生成 `balance_run_summary.csv` 和卡牌漏斗。
