...
- [x] G0：建立 `phase-9b-asset-source-ledger-v0.1.md`；G0 建立时生成式插画与卡框仅限内部工程样板，并冻结“生产许可未确认前不得标记 Runtime Ready 或用于最终发布”的门禁规则。
- [x] G1：冻结纯“旅团绘本（Wandering Storybook）”Style Tile。
  ...
  - [x] 在 Unity 中建立 `PF_BattleStandee` Prefab：完成肖像 Mask、攻防数据绑定、四类关键词锚点、Hover 完整卡牌、点击锁定详情和目标选择接线。
  - [x] 将 v0.5 运行资产接入 `PresentationSpriteCatalog` / `PresentationTheme`，护盾使用 Additive 材质，其他部件使用 Alpha 混合；1×/2× 锚点和现有攻击、伤害、护盾、属性、死亡、召唤事件层已接通。
  - [x] 增加 EditMode / PlayMode 回归并导出双分辨率截图：EditMode 267 / 267、PlayMode 22 / 22 通过；五单位并排、四关键词组合、详情锁定、目标高亮和战斗日志无遮挡见 `ui-concepts/unity-validation/pf-battle-screen-v0.2/`（2026-07-23）。
  - [x] 完成 G1 运行时视觉收口：`PF_Card` 改用暖纸/深墨/靛蓝缝线与局部金箔公共框；战斗区普通立牌改为银灰框、金色立牌保持暖金；合法目标改为空心手绘边线；护盾 Additive 提升至 0.78 并扩大覆盖；移除槽位与立牌根节点矩形背景，只保留立牌、空槽编号和交互热区；补齐卡牌、商店、详情与普通/金色立牌双分辨率证据，评审记录见 `phase-9b-g1-runtime-visual-review-v0.1.md`。
  - [x] 评审共同风格：确认画面温暖轻快但不低龄，Boss、亡语和死亡反馈仍保留威胁感。
  - [x] 评审流派区分：8 张锚点立绘已完成 5 秒盲测并确认无问题；可仅凭轮廓、主色和形状语言区分铸魂、荒灵、星契与旅团，冻结稿与卡面集成验证见 `ui-concepts/phase-9b/archetype-anchor-illustrations-v0.2/`、`ui-concepts/phase-9b/archetype-card-blind-test-v0.2/`。该结论只冻结 G1 工程样板视觉，不等于生产许可或 Runtime Ready。
  - [x] 评审信息可读性：在 Full 240×360、Compact 160×240、商店、战斗和选择层验证名称、等级、攻防、关键词、进度与状态覆盖。
  - [x] 评审金色表现：确认局部暖金墨线、金箔印章和纸面微光足够珍贵，同时保留原流派色和文本对比度。
...
- [ ] G4：完成全量自动化、正式链路双分辨率、存档恢复、跳过/2×、两机性能基线和至少 5 名外部试玩者验收。
  ...
  - [x] 提交 `f377497` 的 Clean Development Build `20260726-g4-f377497` 已冻结：`sourceTreeDirty=false`、`cleanBuild=true`，Build Manifest SHA-256 为 `e09691e14ba931dddade86223527fa30e02dfcc6071a0e08be63bfea12023576`。
  - [x] Frozen 合法 4 店/4 战链完成：双分辨率每组 21 图，覆盖冻结 17 个逻辑状态、地图左/中/右、严格奖励、保存返回与 Continue；没有修改金币或领域状态伪造。
  - [x] Stress 完成同屏 10 张 Compact、嵌套亡语/连续召唤、群体永久成长、Normal/Accelerated/Skip 五轮等价与 30 秒清理稳定窗。
  - [x] 同一候选完成 Core/Stress 双分辨率各 5 轮无截图性能矩阵；26 个正式 Player 运行全部 `AcceptancePassed`、运行时错误 0、样板 Catalog 22 / 22 Exact，证据见 `ui-concepts/unity-validation/g4-formal-chain-v0.1/`。
...
- [x] 冻结明亮主题 v0.3.3：唯一 Style Tile、亮度与近黑门槛、主体—背景分离、
  四阵营身份边界，以及随从、法术、主菜单、商店和地图生产 Prompt 已统一收束；
  不恢复旧随从内容参考或全族身体硬约束。
- [x] 完成四阵营各 3 张正式随从与 3 张法术的隔离 Catalog 样本，共 15 张；
  配置逐字段一致，大图画幅、亮度和人工身份验收通过，未覆盖正式 Runtime 美术。
- [x] 完成离线卡框模拟器与普通/金色 × 紧凑/完整四张 15 卡矩阵；紧凑和完整
  布局均显示费用，用户已完成人工确认。
- [x] 完成 1920×1080 商店与 5 vs 5 战斗压力测试；战斗立绘改为
  `AspectRatioFitter.EnvelopeParent` 居中覆盖裁切，立牌护盾改为独立、
  中心透明的明亮边缘覆盖，并保留原卡面护盾绑定。
